...
addto_host_list() {
	...
	if test $error -eq $true; then
		...
		# WARNING! Nothing to do! Skipping $host! #
		echolog "$WARNING Warning! Nothing to do! Skipping '$host'!";
		# Skip this host $host #
...
load_host_list() {
	...
	if test ! -z "$HOSTS"; then
				...
				if test ! -z "$CHECK_HOST_LIST"; then
					...
					for hf in $CHECK_HOST_LIST; do
						...
						if test $? -eq 0; then
							...
							echolog "$WARNING Warning! '$hf' does not have a DNS name.";
							# Ping $hf
							check_ping $hf;
					...
					echolog "$WARNING Warning! '$host' is not a 'HOST_LIST'!";
		...
		$ECHO
		echolog "$ERROR Error! 'host' or 'HOST_LIST' is needed!";
		$ECHO
		help;
		return $false;
	fi;
...
check_host_list() {
	...
	if test ! -z "$HOST_LIST"; then
		...
		echolog "$ERROR Error! No hosts avaliable!";
		return $false;
	fi;
}
